...
You will go into the M10 modeling environment, using your M10 user account. An extensive access rights infrastructurte will define who can work with what. The environment could be used in two different ways:  
* An interactive, GUI based modeling studio, where you work with your model at your own pace.   
* A scripting tool sandbox, where you could automate tasks, or when yo want to adhere to an IbC methodology of software development.  
Either way is mutualy exchangable and has the identical feature set.  

The most common aspects in the designer tool would be:  
...
3. Define the content of these domain objects. That means; what fields you would need, and what data types are carried by such fields. Other rules could also apply, for instance how some fields should comply to GDPR regulations and/or security measures.  
...
8. Define versioning policies. This is usually always needed, because the actual data that your application is working with, will have meta data that is going to be updated and modiféd. E.g. what should happen with and existing `Customer` record if you determine to add one more field to it later?  

For many of these aspects, it will be possible to test/simulate how the model behaves when feeding it with sample data.
