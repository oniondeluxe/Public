...
In the example above it might be tempting to preemptively conclude that the problem of data conversion and layering can be solved. Maybe, if it was only as simple as using data in a declarative way. But, in real-world applications, we also need to think about the process around how this data is conveyed and reshaped.  
...
* How is the description of the data maintained, and how will this affect already existing data? For instance, what will happen to out existing account ledger, if we introduce a tax column, mandatory for the fiscal year of 2026 and onward? Or, if we deprecate a column due to CDPR compliance?  
