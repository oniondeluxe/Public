...
As a disclaimer, it must incidentally be mentioned that it is of course a challenge to be able to capture all aspects of the condition of a engineering department in such a relatively limited time and resource space. A significant part of the assessment is based on long experience of working in the industry, at different levels, and with different types of interactions between roles and processes.  

## Structure
The survey can be divided into three parts:  

1. The static inventory of the technical platform, i.e. the code base and surrounding technical ecosystems and flows.  

2. Assessments that highlight how people work, internally within the engineering guild.  

3. Studies that illustrate how the collaboration between the different departments works, starting from the engineering department.  
...
- What is the state of the surrounding, scaffolding processes?  
...
In parallel, a template I often use is to simulate some _developer use cases_, i.e. common scenarios in the daily work of the engineering department. For example: describe the procedure, all the way, to add a new column to a business object.  
...
### Interfaces to adjacent departments
The engineering department is a delivery engine in the company. But how well does it fit in with the rest of the business? How expensive and painful is it for other parties in the organization to “conduct business” with the engineering department? Are things being done in the right places and at the right level?
...
The quantification is done with respect to how other departments perceive the engineering department.  

The results are summarized in a graded scale, according to the following guidelines:
...
Once you have captured the overall situation in the production tier, and how well everything fits in with the rest of the business, investigations are done at a more senior level. This involves interviews with the head of the engineering department, the CTO and/or CIO, etc.
...
- How well equipped is the engineering department to deal with competitive pressures? Both from the point of view of agility and resilience, and in terms of recruitment.
...
### Schedule
The schedule is intended to represent a sequence of work phases. For illustrative purposes, it can be assumed to run for a number of days 1–5, but the schedule can also be compressed/expanded depending on the size of the project.
